...
public class ProductEntry {
    ...
    public final static int icon = R.drawable.ic_add_shopping_cart_black_24dp;
    ...
    public static List<ProductEntry> contains(List<ProductEntry> products, final String query) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            for (ProductEntry product : products) {
                products.removeIf(productEntry -> !product.title.toLowerCase().contains(query.toLowerCase()));
            }
        } else {
            List<ProductEntry> filtered = new ArrayList<>();
            for (ProductEntry product : products) {
